...
class PendaftaranLayanan extends \yii\db\ActiveRecord
{
    ...
    function getKelas()
    {
        return $this->hasOne(PendaftaranMKelasRawat::className(),['kr_kode'=>'pl_kr_kode']);
    }
    function getRegistrasi()
    {
        return $this->hasOne(PendaftaranRegistrasi::className(),['reg_kode'=>'pl_reg_kode']);
    }
    function getUnit()
    {
        return $this->hasOne(SdmMUnit::className(),['unt_id'=>'pl_unit_kode']);
    }
    function getKamar()
    {
        return $this->hasOne(MedisMKamar::className(),['kmr_id'=>'pl_kamar_id']);
    }
      ...
      static function searchLayanan($rm)
    {
        ...
        //kunjungan
        $kunjungan = PendaftaranRegistrasi::countKunjungan($rm);
        $kunjungan = $kunjungan==0 ? ++$kunjungan : $kunjungan;
        
        //query layanan
        $query=PendaftaranLayanan::find()->joinWith([
            'unit'=>function($q){
                $q->joinWith([
        ...
        ])->notDeleted(PendaftaranLayanan::$prefix)->orderBy(['pl_tgl_masuk'=>SORT_DESC])->orderBy(['pl_tgl_masuk'=>SORT_DESC])->asArray()->limit(1);
        ...
        if($rawatinap!=NULL){
            ...
            $rawatjalan=$query->andWhere(['or',['pl_jenis_layanan'=>PendaftaranLayanan::$rj],['pl_jenis_layanan'=>PendaftaranLayanan::$igd]])->andWhere("DATE_FORMAT(pl_tgl_masuk,'%Y-%m-%d') = :tgl and pl_tgl_keluar is null",[':tgl'=>date('Y-m-d')])->one();
    ...
    {
        $jenis_layanan=Yii::$app->db->createCommand("SELECT kul_type FROM ".PendaftaranMKelompokUnitLayanan::tableName()." WHERE kul_unit_id = :unit")->bindValues([':unit'=>$this->reg_data->unit])->queryOne();
        ...
        $model = $mlayanan!=NULL ? $mlayanan : new PendaftaranLayanan();
    ...
    function saveRawatjalanLuar()
    {
        $jenis_layanan=Yii::$app->db->createCommand("SELECT kul_type FROM ".PendaftaranMKelompokUnitLayanan::tableName()." WHERE kul_unit_id = :unit")->bindValues([':unit'=>$this->reg_data->unit])->queryOne();

        $jenis=4;
        if($jenis_layanan != Null) {
            $jenis = $jenis_layanan['kul_type'];
        }
        ...
        $model =  new PendaftaranLayanan();
        $model->scenario="rj_create";
        $model->pl_reg_kode=$this->reg_data->reg_kode;
        $model->pl_jenis_layanan=$jenis;
        $model->pl_tgl_masuk=$this->reg_data->reg_tgl_masuk;
        $model->pl_unit_kode=$this->setKodeUnit();
    ...
    {
        $find=false;
        $max_now=self::find()->where(['pl_unit_kode'=>$this->pl_unit_kode,"DATE_FORMAT(pl_tgl_masuk,'%Y-%m-%d')"=>date('Y-m-d')])->notDeleted(self::$prefix)->max('pl_nomor_urut');
        $max= !empty($max_now) ? $max_now : 1;
        while(!$find){
            $count=self::find()->where(['pl_nomor_urut'=>$max,'pl_unit_kode'=>$this->pl_unit_kode,"DATE_FORMAT(pl_tgl_masuk,'%Y-%m-%d')"=>date('Y-m-d')])->notDeleted(self::$prefix)->count();
            if($count<1){
                $find=true;
            }else{
                $max++;
